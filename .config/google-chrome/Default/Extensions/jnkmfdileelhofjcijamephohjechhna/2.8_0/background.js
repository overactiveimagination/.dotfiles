...
function updateBrowserAction() {
  ...
  chrome.browserAction.setIcon({
    path: debug ? {'16': 'icon-on-16.png', '32': 'icon-on-32.png'} :
                  {'16': 'icon-off-16.png', '32': 'icon-off-32.png'}
  });
...
hasAPIs && chrome.webRequest.onBeforeRequest.addListener(function(details) {
  if (debug) {
    ...
    if (u == GA_HTTP){
      return {redirectUrl: GA_HTTP_D};
    } else if (u == GA_HTTPS) {
      return {redirectUrl: GA_HTTPS_D};
    } else if (u.indexOf('dc.js') > 0) {
      return {redirectUrl: p + DC_HTTP_D};
    } else if (u.indexOf('analytics.js') > 0) {
      return {redirectUrl: p + UA_HTTP_D};
    } else if (u.indexOf('/gtag/js') > 0 && u.indexOf('dbg=') < 0) {
      const separator = (u.indexOf('?') > 0) ? '&' : '?';
      return {redirectUrl: u + separator + 'dbg=' + generateCacheBuster()};
    }
  }
}, {urls: [GA_HTTP, GA_HTTPS, DC_HTTP, UA_HTTP, GTAG_HTTP]}, ['blocking']);
...
}, {urls: [GTAG_HTTP]}, ['blocking', 'requestHeaders', 'extraHeaders']);
